	...
	<div class="forms_table widefat mp6-primary wp-list-table fixed posts">
		...
		<section>
			...
			if(get_option('widget_widget_leadsius_form') ){
				?>
				<?php foreach (get_option('widget_widget_leadsius_form') as $key=>$form): ?>

                    <?php
                    //var_dump(get_option('widget_widget_leadsius_form'));die();

                    ?>

					<?php if($key!='_multiwidget' /*&& (isset($form['category']) && ($form['category']=="shortcode"))*/ ):?>	
						<?php $id='widget_leadsius_form-'.$key; ?>
						<div id="form-<?php echo $key;?>" class="clearfix form-row widefat">
							...
							if($form['category']=="shortcode"){
								...
								<span class="col2 column-shortcode">
									[leadsius-form id="<?php echo $key; ?>"]</span> 
									<?php 				
								}else{
									?>
				...
				<div class="cont_new_button"><button class="new_webform button-primary">Add new shortcode form</button></div>
						...
						<?php if ( isset($insights_leadsius_forms->webforms) && is_array($insights_leadsius_forms->webforms) && count($insights_leadsius_forms->webforms) != 0 ): ?>
							...
							<div class="widget-config">
								...
								<p>
									...
									<select class="widefat" name="form">
					...
					<div class="widget-config">
						...
						<p>
							...
							<select class="widefat" name="form">
								...
								<option value="{{this.id}}"   {{#is this.selected "selected"}} selected="selected" {{/is}}>{{this.name}}</option>
						...
						<p class="select_colors" {{#is 1.show_color true}} style="display:block" {{/is}}>
							<label>Colors</label>
							<select class="widefat" name="color">
								<option value="none">-- none --</option>
								{{#each 1.color}}
			...
			<script id="form-row-template" type="text/x-handlebars-template">
				...
				<div id="form-{{id}}" class="clearfix form-row widefat">
					...
					<span class="col2 column-shortcode">[leadsius-form id="{{id}}"]</span>
					<span class="col3">shortcode</span>
					<div class="col4">
						<button class="edit button">Edit</button><button class="copy button">Copy</button><button class="delete button">Delete</button><span class="spinner"></span>
					</div>

					<div class="edit_this_form" style="display:none">
					</div>
				</div>
				...
				<script  type='text/javascript'>
					...
					$(document).ready(function(){
						...
						$(document).on("click",".scwu",function(e){
							...
							console.log(parent);
							var par='#'+parent;
							imageSourceField=$(par).find('.scwu').siblings('input');
				...
				jQuery(document).ready(function($){
					...
					jQuery( document ).on( "click", ".save_new_form", function(e) {
						...
						jQuery.ajax({
							...
							url: ajaxurl,
							data: {
								action: 'web_form_settings_save_new',
								datos: ser
							},
							beforeSend: function() {
					...
					$(document).on("click",".delete",function(event) {
						...
						jQuery.ajax({
							...
							url: ajaxurl,
							data: {
								action: 'web_form_settings_delete',
								id_form: id[1]
							},
							beforeSend: function() {
					...
					$(document).on("click",".update_form",function(event) {
						...
						jQuery.ajax({
							...
							url: ajaxurl,
							data: {
								action: 'web_form_settings_update',
								id_form: id[1],
								datos:ser
							},
							beforeSend: function() {
